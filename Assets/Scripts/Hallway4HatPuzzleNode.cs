...
public class Hallway4HatPuzzleNode : MonoBehaviour
{
    ...
    [SerializeField] private Hallway4HatPuzzleNode leftZone;
    [SerializeField] private Hallway4HatPuzzleNode rightZone;
    [SerializeField] private Hallway4HatPuzzleNode upZone;
    [SerializeField] private Hallway4HatPuzzleNode downZone;
    [SerializeField] private Hallway4HatPuzzleNode unlockTarget;
    ...
    {
        if (leftZone == null || rightZone == null || upZone == null || downZone == null || unlockTarget == null)
        {
            Debug.LogWarning($"[{name}] Faltan referencias del puzzle.", this);
            return;
        }

        bool leftOccupied = leftZone.IsOccupied();
        bool rightOccupied = rightZone.IsOccupied();
        bool upOccupied = upZone.IsOccupied();
        bool downOccupied = downZone.IsOccupied();

        if (verboseLogs)
        {
            Debug.Log(
                $"[{name}] EvaluatePuzzle -> left:{leftOccupied} right:{rightOccupied} up:{upOccupied} down:{downOccupied}",
                this
            );
        }

        if (leftOccupied && rightOccupied && upOccupied && downOccupied)
        {
